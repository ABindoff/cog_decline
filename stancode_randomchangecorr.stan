data {
  ...
  int<lower=0,upper=1> cov[N];    // a covariate
}

parameters {
  vector[4] beta;            // fixed effects, intercept and slopes, covariate
...
transformed parameters {  
  ...
  for (j in 1:N) {
    if (age[j] < alpha[id[j],4]) 
      y_mu[j] = cov[j] + alpha[id[j],1] + alpha[id[j],2] * (age[j] - alpha[id[j],4]);
    else  
      y_mu[j] = cov[j] + alpha[id[j],1] + alpha[id[j],3] * (age[j] - alpha[id[j],4]);     
  } 
}
...
model {
  ...
  beta[4] ~ normal(0, 4);  // prior: covariate
...
generated quantities {
  ...
  for (j in 1:Npred) {
    if (age_pred[j] < alpha[id_pred[j],4]) 
      y_mu_pred[j] = cov[j] + alpha[id_pred[j],1] + alpha[id_pred[j],2] * (age_pred[j] - alpha[id_pred[j],4]);
    else  
      y_mu_pred[j] = cov[j] + alpha[id_pred[j],1] + alpha[id_pred[j],3] * (age_pred[j] - alpha[id_pred[j],4]);      
