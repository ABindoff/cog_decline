data {
  ...
  int<lower=0> Ntest;         // number of unique tests  
  real y[N];        // outcome data
  real<lower=0> tau[N];      // explanatory variable (tau) data
  int<lower=1,upper=Ntest> id[N]; // test id for each observation
  ...
  real<lower=0> tau_pred[Npred]; // explanatory variable (tau) for prediction
  int<lower=1,upper=Ntest> id_pred[Npred]; // test id for each predicted observation
}
...
transformed parameters {  
  ...
  for (j in 1:N) {
    if (tau[j] < alpha[id[j],4]) 
      y_mu[j] = alpha[id[j],1] + alpha[id[j],2] * (tau[j] - alpha[id[j],4]);
    else  
      y_mu[j] = alpha[id[j],1] + alpha[id[j],3] * (tau[j] - alpha[id[j],4]);     
  } 
}
...
model {
  ...
  beta[3] ~ normal(-1, 0.5);   // prior: fixed effect, slope after knot
  ...
  for (i in 1:Ntest) u[i] ~ multi_normal_cholesky(zeros4, diag_pre_multiply(u_sd, L_u_Corr));
...
generated quantities {
  ...
  vector[4] alpha_tosave[Ntest_pred];
  ...
  for (j in 1:Npred) {
    if (tau_pred[j] < alpha[id_pred[j],4]) 
      y_mu_pred[j] = alpha[id_pred[j],1] + alpha[id_pred[j],2] * (tau_pred[j] - alpha[id_pred[j],4]);
    else  
      y_mu_pred[j] = alpha[id_pred[j],1] + alpha[id_pred[j],3] * (tau_pred[j] - alpha[id_pred[j],4]);      
